...
# Problem Statement
 How have house prices in U.S. states changed over the last few decades, and are changes in prices are related to population in some way?
...
## Question 1: House prices over time:
...
```
The house prices were fluctuating till 1998 after removing the inflation, it peaked at 1980 and 1990. After 1998, the house prices grew rapidly till 2006 and peaked at just below 200 after that it declined in next five years. We think that this growth in house price can be attributed to the housing bubble which was developed between 2000-2006 and once the bubble got burst the prices fell abruptly.


We calculated inflation based on --------

```{r, warning=FALSE,echo=FALSE, fig.align='center', fig.width=4, fig.height=4}
ggplot(cpi_data,aes(x = Year , y = US))+geom_line()+ geom_smooth()+ labs(title = "Year v/s Change in HPI(Inflation Adjusted)",x=" Year",y="HPI (Inflation Adjusted")  
```
...
northeast_plot <- ggplot(northeast_mean, aes(x=Year, y = value)) + geom_line() + labs(y = "HPI", x = "Year")
...
south_plot <- ggplot(south_mean, aes(x=Year, y = value)) + geom_line() + labs(y = "HPI", x = "Year")
...
```{r, echo=FALSE, fig.align='center', fig.width=3, fig.height=3}
west_plot <- ggplot(west_mean, aes(x=Year, y = value)) + geom_line() + labs(y = "HPI", x = "Year")
...
```{r, echo=FALSE, fig.align='center', fig.width=3, fig.height=3}
midwest_plot <- ggplot(midwest_mean, aes(x=Year, y = value)) + geom_line() + labs(y = "HPI", x = "Year")
...
```{r, echo=FALSE, fig.align='center', fig.width=3, fig.height=3}
g1 = ggplot(dc_df, aes(x = Year, y = value)) + geom_line() + geom_smooth(method = "lm", se = F)+ ggtitle("DISTRICT OF COLUMBIA") + labs(y = "HPI", x = "Year") +ylim(c(25,350))
g2 = ggplot(cal_df, aes(x = Year, y = value)) + geom_line() + geom_smooth(method = "lm", se = F)+ ggtitle("CALIFORNIA") + labs(y = "HPI", x = "Year") +ylim(c(25,350))
g3 = ggplot(hi_df, aes(x = Year, y = value)) + geom_line() + geom_smooth(method = "lm", se = F)+ ggtitle("HAWAII") + labs(y = "HPI", x = "Year") +ylim(c(25,350))
g4 = ggplot(wa_df, aes(x = Year, y = value)) + geom_line() + geom_smooth(method = "lm", se = F)+ ggtitle("WASHINGTON")+ labs(y = "HPI", x = "Year") +ylim(c(25,350))
...
```{r, echo=FALSE, fig.align='center', fig.width=3, fig.height=3}
g1 = ggplot(ar_df, aes(x = Year, y = value)) + geom_line() + geom_smooth(method = "lm", se = F)+ ggtitle("ARKANSAS") + labs(y = "HPI", x = "Year") +ylim(c(100,225)) 
g2 = ggplot(oh_df, aes(x = Year, y = value)) + geom_line() + geom_smooth(method = "lm", se = F)+ ggtitle("OHIO") + labs(y = "HPI", x = "Year") +ylim(c(100,225))
g3 = ggplot(ok_df, aes(x = Year, y = value)) + geom_line() + geom_smooth(method = "lm", se = F)+ ggtitle("OKLOHOMA") + labs(y = "HPI", x = "Year") +ylim(c(100,225))
g4 = ggplot(wv_df, aes(x = Year, y = value)) + geom_line() + geom_smooth(method = "lm", se = F)+ ggtitle("WEST VIRGINIA") + labs(y = "HPI", x = "Year") +ylim(c(100,225))
grid.arrange(g1,g2,g3,g4, nrow = 2)
```

d. Have changes in prices within each state mostly followed the same basic pattern, and are there outliers to that pattern? 

Majority  of the states follow the same basic pattern where they see rise in house prices in 1980, 1990 and 2006 and decline after that. But there few states which do not follow these pattern like West Virgina, North Dacota,District of Columbia, Washington are the few ones
...
```{r, echo=FALSE, fig.align='center', fig.width=4, fig.height=4}
g1 = ggplot(west_mean1, aes(x = Year, y = value)) + geom_line() + ggtitle("WEST") + labs(y = "HPI", x = "Year") +ylim(c(100,225))
g2 = ggplot(midwest_mean1, aes(x = Year, y = value)) + geom_line() + ggtitle("MIDWEST") + labs(y = "HPI", x = "Year") +ylim(c(100,225))
g3 = ggplot(northeast_mean1, aes(x = Year, y = value)) + geom_line() + ggtitle("NORTHEAST")+ labs(y = "HPI", x = "Year") +ylim(c(100,225))
g4 = ggplot(south_mean1, aes(x = Year, y = value)) + geom_line() + ggtitle("SOUTH")+ labs(y = "HPI", x = "Year") +ylim(c(100,225))
grid.arrange(g1,g2,g3,g4, nrow = 2)
```
